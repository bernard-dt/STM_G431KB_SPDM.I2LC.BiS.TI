...
uint32_t u32TickFreq = HAL_TICK_FREQ_DEFAULT;  /* 1KHz */
/**
  * @}
  */
...
__weak HAL_StatusTypeDef HAL_InitTick(uint32_t TickPriority)
{
  ...
  if (u32TickFreq != 0U)
  {
    ...
    if (HAL_SYSTICK_Config(SystemCoreClock / (1000U / u32TickFreq)) == 0U)
    {
...
HAL_StatusTypeDef HAL_SetTickFreq(uint32_t Freq)
{
  ...
  if (u32TickFreq != Freq)
  {
    /* Back up u32TickFreq frequency */
    prevTickFreq = u32TickFreq;

    /* Update u32TickFreq global variable used by HAL_InitTick() */
    u32TickFreq = Freq;

    /* Apply the new tick Freq  */
    status = HAL_InitTick(u32TickPrio);

    if (status != HAL_OK)
    {
      /* Restore previous tick frequency */
      u32TickFreq = prevTickFreq;
    }
  }
...
/**
  ...
  * @note In the default implementation , SysTick timer is the source of time base.
  *       It is used to generate interrupts at regular time intervals where u32Tick
  *       is incremented.
  * @note This function is declared as __weak to be overwritten in case of other
  *       implementations in user file.
